...
(function applyRaamit() {
  ...
  Promise.all([header, footer]).then(function(values) {
    ...
    if (getLanguage() === 'fi') {
      var buText = 'Selaimesi {brow_name} on vanhentunut. Päivitä selaimesi turvallisempaan, nopeampaan ja helppokäyttöisempään. <a{up_but}>Päivitä selain</a><a{ignore_but}>Hylkää</a>'
    } else if (getLanguage() === 'sv') {
      var buText = 'Din webbläsare {brow_name} är föråldrad. Uppdatera din webbläsare för mer säkerhet, snabbhet och den bästa upplevelsen. <a{up_but}>Uppdatera webbläsaren</a><a{ignore_but}>Ignorera</a>'
    } else {
      var buText = 'Your web browser {brow_name}, is out of date. Update your browser for more security, speed and the best experience. <a{up_but}>Update browser</a><a{ignore_but}>Ignore</a>'
    }
    browserUpdate({required:{e:-4,f:-3,o:-3,s:-1,c:-3},
      insecure:true,
      unsupported:true,
      test:true,
      text:buText,
